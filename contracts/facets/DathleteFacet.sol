...
import {LibDathlete, DathleteInfo} from "../libraries/LibDathlete.sol";
...
contract DathleteFacet {
    ...
    function balanceOf(address _owner) external view returns (uint256 balance_) {
        require(_owner != address(0), "DathleteFacet: _owner can't be address(0");
    ...
    ///@return dathleteInfo_ a struct containing all details about
    function getDathlete(uint256 _tokenId) external view returns (DathleteInfo memory dathleteInfo_) {
        dathleteInfo_ = LibDathlete.getDathlete(_tokenId);
    }
    ...
    function tokenByIndex(uint256 _index) external view returns (uint256 tokenId_) {
        require(_index < s.tokenIds.length, "DathleteFacet: index beyond supply");
    ...
    function tokenOfOwnerByIndex(address _owner, uint256 _index) external view returns (uint256 tokenId_) {
        require(_index < s.ownerTokenIds[_owner].length, "DathleteFacet: index beyond owner balance");
    ...
    /// @param _owner The address to check for the NFTs
    /// @return dathleteInfos_ an array of structs,where each struct contains all the details of each NFT
    function allDathletesOfOwner(address _owner) external view returns (DathleteInfo[] memory dathleteInfos_) {
        uint256 length = s.ownerTokenIds[_owner].length;
        dathleteInfos_ = new DathleteInfo[](length);
        for (uint256 i; i < length; i++) {
            dathleteInfos_[i] = LibDathlete.getDathlete(s.ownerTokenIds[_owner][i]);
        }
    }
    ...
    function ownerOf(uint256 _tokenId) external view returns (address owner_) {
        owner_ = s.dathletes[_tokenId].owner;
        require(owner_ != address(0), "DathleteFacet: invalid _tokenId");
    }
    ...
    function internalTransferFrom(
        ...
        require(_to != address(0), "DathleteFacet: Can't transfer to 0 address");
        require(_from != address(0), "DathleteFacet: _from can't be 0 address");
        require(_from == s.dathletes[_tokenId].owner, "DathleteFacet: _from is not owner, transfer failed");
        require(
            _sender == _from || s.operators[_from][_sender] || _sender == s.approved[_tokenId],
            "DathleteFacet: Not owner or approved to transfer"
        );
        LibDathlete.transfer(_from, _to, _tokenId);
    }
    ...
    function approve(address _approved, uint256 _tokenId) external {
        address owner = s.dathletes[_tokenId].owner;
